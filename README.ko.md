...
5. 이 확장 파일 `theclawbay-for-copilot-0.2.0.vsix`
...
3. 확장 패널 위쪽 `...` 메뉴를 누릅니다.
4. **VSIX에서 설치...** 를 선택합니다.
5. `theclawbay-for-copilot-0.2.0.vsix` 파일을 선택합니다.
...
```powershell
code --install-extension "C:\전체경로\theclawbay-for-copilot-0.2.0.vsix" --force
```

macOS / Linux:

```bash
code --install-extension "/전체/경로/theclawbay-for-copilot-0.2.0.vsix" --force
```
